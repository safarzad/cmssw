...
void L1JetRecoTreeProducer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 ...
 // std::cout<<chStatus.isValid()<<std::endl;
...
*/
    ...
    if (flag>ecalFlag) ecalFlag = flag;        
          met_data->sevlT.push_back(sevlv->severityLevel( eid, *ebRecHits));

	  if  (flag ==4) ecalSpikeFlag = flag;        
         // met_data->sevlT.push_back(sevlv->severityLevel( eid, *ebRecHits));

  }
  
 //   std::cout<<"we are here:"<<ecalFlag<<","<<ecalSpikeFlag<<std::endl;

   met_data->ecalFlag = ecalFlag;
   met_data->ecalSpikeFlag = ecalSpikeFlag;
